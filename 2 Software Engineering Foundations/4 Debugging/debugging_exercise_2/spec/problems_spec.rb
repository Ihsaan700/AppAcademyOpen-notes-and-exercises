...
describe 'dupe_indices' do
  it 'should accept an array as an arg' do
    expect { dupe_indices(%w[a b c c b]) }.to_not raise_error
  end

  it 'should return a hash where keys are the elements that were repeated in the array and values are the indices where that element appears' do
...
describe 'ana_array' do
  it 'should accept two arrays as args' do
    expect { ana_array(%w[i c e m a n], %w[c i n e m a]) }.to_not raise_error
  end
